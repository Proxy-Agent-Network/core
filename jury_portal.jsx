...
// --- v2.18 NEW: Secure Message Tunnel ---
    ...
    <div className="p-4 border-t border-white/10 bg-white/[0.01]">
       <div className="flex gap-2">
          ...
          <button 
            ...
            className="p-2 bg-blue-500/10 border border-blue-500/30 text-blue-400 rounded hover:bg-blue-500 hover:text-black transition-all disabled:opacity-20"
          >
            <Send className="w-4 h-4" />
          </button>
       </div>
       <p className="text-[8px] text-gray-600 mt-2 uppercase text-center font-bold italic tracking-widest">
          "Messages are cryptographically signed by TPM. Logged for Tribunal Audit only."
       </p>
    </div>
  </div>
);
...
const App = () => {
  ...
  // v2.18 Messaging State
  const [newMessage, setNewMessage] = useState("");
  const [messages, setMessages] = useState([
    { sender: "AGENT", content: "Tunnel established. We are providing evidence for CASE-885-4.", timestamp: "12:12:05" },
    { sender: "AGENT", content: "Proof blob hash confirms Verizon Wireless network origin.", timestamp: "12:12:30" }
  ]);
                 ...
                 {cases.map((c) => (
                    <div key={c.id} className="p-6 flex items-center justify-between hover:bg-white/[0.01] transition-colors group">
                       <div className="flex items-center gap-6">
                          ...
                          <div><span className="text-white font-bold uppercase tracking-tighter">{c.id}</span><p className="text-xs text-gray-500">{c.dispute_reason}</p></div>
                       </div>
