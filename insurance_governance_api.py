...
const App = () => {
  ...
  // Mock Proposals (Reflecting logic in core/ops/insurance_governance_api.py)
  ...
  const executeVote = () => {
    setIsVoting(true);
    // Simulate TPM 2.0 RSA-PSS Signing Ceremony
    setTimeout(() => {
      setIsVoting(false);
      setHasVoted(true);
    }, 2500);
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[10px] text-gray-600 uppercase tracking-widest font-bold mt-2">
                Economic Adjudication // High Court Juror: <span className="text-indigo-400">NODE_ELITE_X29</span>
              </p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-12 gap-8 items-stretch">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6">
             <div className="bg-[#0a0a0a] border border-white/5 rounded-xl overflow-hidden shadow-2xl flex flex-col min-h-[500px]">
                ...
                <div className="divide-y divide-white/5 overflow-y-auto max-h-[600px] scrollbar-hide">
                   {proposals.map((prop) => (
                      <div 
                        key={prop.id} 
                        onClick={() => { setSelectedProposal(prop); setHasVoted(false); }}
                      ...
                      >
                         ...
                         <div className="space-y-3">
                            <div className="flex justify-between items-center text-[8px] font-black text-gray-700 uppercase">
                               <span>Quorum Progress: {prop.votes_signed}/{prop.votes_required}</span>
             ...
             <div className="p-8 border border-white/10 bg-indigo-500/5 rounded-xl shadow-inner relative overflow-hidden group">
                ...
                <p className="text-[11px] text-gray-500 leading-relaxed italic">
                   "Economic manual overrides must achieve a 5/7 super-majority consensus. Every vote is cryptographically bound to the juror's TPM identity key, creating a permanent judicial record of market intervention."
                </p>
             </div>
          </div>

          {/* Adjudication View (Right Column) */}
          <div className="col-span-12 lg:col-span-7 flex flex-col">
             {selectedProposal ? (
                         ...
                         <div className="grid grid-cols-2 gap-4">
                            <div className="p-5 bg-white/[0.02] border border-white/5 rounded-xl">
                               ...
                               <span className="text-xs font-black text-white flex items-center gap-2">
                                  <Globe className="w-3 h-3 text-blue-500" /> {selectedProposal.region}
                               </span>
                            </div>
                            <div className="p-5 bg-white/[0.02] border border-white/5 rounded-xl">
                      ...
                      <div className="space-y-8">
                         ...
                         <div className="p-6 bg-red-500/5 border border-red-500/10 rounded-xl">
                            ...
                            <p className="text-[11px] text-gray-500 leading-relaxed font-bold">
                               Approving this override will manually bypass the Actuary Engine's calculated surcharge for the next <span className="text-white">24 hours</span>.
                            </p>
                         </div>
                      </div>
                   </div>
                      ...
                      {!hasVoted ? (
                        <>
                          <div className="flex items-center gap-6 flex-1 bg-white/[0.02] border border-white/5 p-6 rounded-xl shadow-inner">
                             ...
                             <div>
                                ...
                                <p className="text-[10px] text-gray-600 leading-relaxed font-medium">
                                   Awaiting signature for <span className="text-white">NODE_ELITE_X29</span>. Your vote will be immutable once broadcast.
                                </p>
                             </div>
                          </div>
                          <div className="flex gap-4 w-full md:w-auto">
                             <button 
                               onClick={executeVote}
                               disabled={isVoting}
                               className="px-10 py-5 bg-white text-black font-black text-xs uppercase tracking-[0.2em] hover:bg-indigo-500 hover:text-white transition-all rounded shadow-2xl flex items-center justify-center gap-3 whitespace-nowrap"
                             >
                                {isVoting ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Fingerprint className="w-4 h-4" />}
                                Sign & Broadcast
                             </button>
                             <button 
                               className="px-10 py-5 bg-white/5 border border-white/10 text-gray-500 font-black text-xs uppercase tracking-widest hover:text-red-500 transition-all rounded"
                             >
                                Reject
                             </button>
                          </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center text-center animate-in zoom-in-95 duration-500">
                           ...
                           <p className="text-xs text-gray-500 max-w-sm mx-auto leading-relaxed">
                              Your adjudication signature has been anchored to the High Court registry. Awaiting remaining {selectedProposal.votes_required - (selectedProposal.votes_signed + 1)} jurors.
                           </p>
                           <div className="mt-8 p-4 bg-black border border-white/5 rounded-xl mono text-[10px] text-gray-700">
                              SIG_RECEIPT: 0x8A2E...{Math.random().toString(16).slice(2, 10).toUpperCase()}
                           </div>
                        </div>
                      )}
                   </div>
                ...
                <div className="h-full border-2 border-white/5 border-dashed rounded-2xl flex flex-col items-center justify-center p-20 text-center opacity-40 grayscale">
                   ...
                   <p className="text-sm text-gray-800 font-bold uppercase tracking-widest max-w-md">Select an economic proposal from the docket to initiate forensic review and cast your hardware-bound judicial vote.</p>
                </div>
             )}
          </div>
