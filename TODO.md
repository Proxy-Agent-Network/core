...
Items tagged **[PILOT BLOCKER]** must be resolved before the Vanguard 50 Mesa Pilot goes live. There are currently **0 active pilot blockers** (down from 9 at peak). All 9 have been resolved or deferred: the 2026-04-26 sweep resolved 4, commits `d0abeb0`/`21dbf77`/`2a44123` on 2026-04-26 resolved the remaining 4 (Firebase Auth, telemetry auth, AZ mandate, fee math UI), and the HapHat hardware blocker was deferred to the Q3 2026 to Q2 2027 hardware rollout per the pilot pivot. Search for that tag to review resolution history.
...
The repository has completed its pivot from the original "Proxy AI" cognitive-vault Flask monolith to the "Proxy Agent Network" FastAPI V2X gateway for the Mesa autonomous-vehicle-recovery pilot. The Flask monolith (`app.py`), root `Dockerfile`, and root `docker-compose.yml` were deleted in Stage 1d-5 (commit `ce6cdf8`, 2026-04-26).
...
* `core/` — 75 tracked files remaining (down from 444 at start of migration). The v2 "AI civilization" backend code has been fully purged. Remaining content is mostly pre-pivot infrastructure (requirements.txt, Cargo.toml), pre-pivot Python entrypoints (master_node.py, mcp_server.py, dashboard.py, agent_engine_v2.py, autonomous_worker.py), and legacy subdirectories that may overlap with canonical locations elsewhere in the repo. See the Repo Hygiene / Architecture section for the residual cleanup plan.
* ~~`apps/backend/entrypoints/app.py`~~ — **DELETED in Stage 1d-5** (commit `ce6cdf8`, 2026-04-26). All endpoints have been migrated to FastAPI routers under `apps/backend/src/api/` or inline in `main.py`. The `/seed-dvr` dev tool was salvaged to `dev_api.py`. The only surviving entrypoint is `mcp_server.py`.
* ~~Root `Dockerfile` and `docker-compose.yml`~~ — **DELETED in Stage 1d-5** (commit `ce6cdf8`, 2026-04-26). Both built the pre-pivot Flask + cognitive vault + LND + bitcoind stack. A lightweight replacement compose for postgres/redis-only local dev has not yet been created.

**Canonical evidence-upload path (added 2026-04-25, updated 2026-05-21):** `apps/backend/src/api/agent_api.py` defines the live, hardened `/agent/evidence/upload` endpoint. It is wired into `main.py` at `/api/v1` prefix (final URL `/api/v1/agent/evidence/upload`), is what the Android client calls (`PanApiClient.kt` line 393), and is the production-quality implementation: rate limiting (60/hr/agent), magic-byte content sniffing for JPEG/PNG verification, AES256 server-side encryption, S3 Object Lock in COMPLIANCE mode with 12-month retention for SB 1417 §28-9710 WORM compliance (added in commit `17bad8d`), SHA-256 content-addressable hashing, S3 object metadata for chain of custody, presigned URL generation, a Redis ownership index at `pan:agent:{agent_id}:evidence:{blob_id}` used by the mission-completion handler to verify agent ownership of submitted evidence URLs, and a production-blocking startup check that raises `RuntimeError` if `S3_EVIDENCE_BUCKET_NAME` env var is unset in production. The legacy `evidence_api.py` was deleted in Stage 1e-3 (commit `a34123e`) as a half-finished alternative implementation.
...
* **Stage 1d-3-d-1** (`5ac8727`): First endpoint of Stage 1d-3-d migrated. Created `apps/backend/src/api/dispatch_api.py` and migrated `/api/v1/dispatch/request` from Flask to FastAPI with constant-time `PARTNER_API_KEY` bearer auth via a local `verify_partner_api_key` dependency. Behavior preserved bit-for-bit vs the legacy Flask handler: same FLT-NNNNN mission ID format, same tier and bounty math (Tier 1 = $15, Tier 2 = $25 for spill_remediation/tire_pressure, Tier 3 = $85 for manual_override/scene_securement, max cap = base + $20), same 400 contract for missing fields and invalid JSON (manual dict validation chosen over Pydantic specifically to preserve this; Pydantic default would be 422), same 401 contract for missing/invalid auth, same 201 response shape and timestamp wire format. New behavior added during migration: synthesized mission_id is now published to `pan:stream:distress_alerts` Redis channel so the Ops Hub command-center map lights up on partner-originated dispatches. The legacy Flask handler built this `map_payload` dict but never sent it; the downstream subscriber in `api/telemetry_socket.py` was already in place. Smoke tested: 10-curl sequence covering 401 (missing/wrong/no-Bearer-prefix), 400 (empty body, missing field), and 201 across all three tiers with correct escrow and max_cap math. Redis publish verified live via `redis-cli MONITOR`. 2 files changed, 224 insertions.
* **Stage 1d-3-d-2** (`21dbf77`): Migrated `/api/v1/telemetry/history` from Flask to FastAPI. Added strict `OPS_HUB_TOKEN` verification dependency to prevent unauthenticated access to the forensic GPS ledger, resolving a security issue flagged during earlier reviews. The `?global=true` parameter now requires authenticated ops-hub access. 1 file changed, 80 insertions, 25 deletions.
* **Firebase Auth E2E** (`d0abeb0`): Implemented real Firebase Auth end-to-end flow in the mobile app. Wired `FirebaseAuth` into `LoginScreen.kt` with email/password authentication, updated `PanWalletClient.kt` to resolve agent identity from `FirebaseAuth.getInstance().currentUser` instead of falling back to `DEV_AGENT_01`. Updated `AgentDashboardScreen.kt` to read authoritative `netPayout` from the backend mission completion response instead of computing fees locally with the hardcoded `isVeteran = true` mock. 4 files changed, 43 insertions, 27 deletions.
* **AZ M2H Mandate + Fee Docs** (`2a44123`): Drafted `docs/legal/AZ_M2H_MANDATE.md` (68 lines) — the Arizona Machine-to-Human Physical Intervention Mandate required for the Mesa Pilot under AZ Rev Stat § 28-9701 (SB 1417). Covers authorization and appointment, scope of authority (ORP, perimeter clear, cabin/closure secure, scene secure), strict prohibitions (slashing offenses), liability shield with millisecond-precision UWB transfer, and term/expiration conditions. Updated `docs/API_SPEC.md` and `docs/legal/COMPLIANCE.md` to reflect the canonical tiered fee structure (15%/25% agent, 5%/10% platform with escrow threshold), replacing the old flat 10% references. 3 files changed, 119 insertions, 49 deletions.
* **Stage 1d-5 / Flask Retirement** (`ce6cdf8`): **Completed the Flask-to-FastAPI migration.** Deleted `apps/backend/entrypoints/app.py` (1,007 lines), root `Dockerfile` (83 lines), and root `docker-compose.yml` (109 lines). Dropped the obsolete worker mock endpoints (`/api/v1/node/register`, `/api/v1/task/request`, `/api/v1/task/submit`). Salvaged the `/seed-dvr` Manhattan-grid telemetry seeder to `apps/backend/src/api/dev_api.py` (91 lines). Extracted the duplicated `decode_redis_hash` helper to `apps/backend/src/utils/redis_helpers.py`. Added `load_dotenv()` to `main.py` for developer convenience. Pinned Python version in `.python-version`. 9 files changed, 114 insertions, 1,275 deletions.
* **Dev Cleanup** (`1a9bac8`, `d958119`): Purged global Redis keys on dev reset to prevent stale state between sessions. Fixed hardware attestation mismatch bypass and cleaned up residual fee UI references in the mobile app. 2 commits spanning 2026-04-27.
* **Docs Update** (`3009d79`): Updated project summary and added compliance vision docs. 2026-05-01.

Cumulative deletion across all stages: ~485 files, ~60,300 lines deleted from the repo (including the 1,007-line Flask monolith, 83-line Dockerfile, and 109-line docker-compose.yml removed in Stage 1d-5). The Flask-to-FastAPI migration is **COMPLETE**. `apps/backend/src/api/` now contains 11 files, all using proper APIRouter pattern: agent_api.py, dev_api.py (NEW, salvaged from app.py), dispatch_api.py, network_stats_api.py, onboarding_api.py, reports_api.py, store_api.py, telemetry_socket.py, v2x_bounty_api.py, v2x_telemetry_api.py, wallet_api.py (ten wired into main.py directly, the eleventh — v2x_telemetry_api — imported transitively via v2x_bounty_api). The sole surviving entrypoint in `apps/backend/entrypoints/` is `mcp_server.py` (the FastMCP server for fleet partner AI clients — live and working, not legacy).

### Stage 1d-3 — COMPLETE: command_center backend migrated from Flask to FastAPI

All substages of Stage 1d-3 have been completed. The Flask monolith at `apps/backend/entrypoints/app.py` has been fully superseded and deleted. See the completed-stages history above for per-substage details. Summary of final-session resolutions:

* `/api/v1/dispatch/request` — migrated in `5ac8727` (Stage 1d-3-d-1).
* `/api/v1/telemetry/history` — migrated and secured with `OPS_HUB_TOKEN` auth in `21dbf77` (Stage 1d-3-d-2).
* `/api/v1/node/register`, `/api/v1/task/request`, `/api/v1/task/submit` — **deleted** in `ce6cdf8`. These mock worker endpoints that 403'd in production were dropped entirely during Stage 1d-5.
* `/seed-dvr` — **salvaged** to `apps/backend/src/api/dev_api.py` in `ce6cdf8`.
* `/api/v1/admin/settings` — dropped along with app.py. The theme/mood_intensity settings were vestigial from the pre-pivot UI and are not used by the post-pivot command center.

### Stage 1d-5 — COMPLETE: app.py + Dockerfile + docker-compose.yml retired

Completed in commit `ce6cdf8` (2026-04-26). All items accomplished:

* ~~Delete `apps/backend/entrypoints/app.py`.~~ DONE. 1,007 lines deleted.
* ~~Delete the now-dead `load_optional_router` helper in `main.py`.~~ DONE (removed with app.py deletion).
* ~~Delete the root `Dockerfile`.~~ DONE. 83 lines deleted.
* ~~Delete the root `docker-compose.yml`.~~ DONE. 109 lines deleted. A lightweight replacement compose for postgres/redis-only local dev has NOT yet been created — this is a future quality-of-life item.
* `decode_redis_hash` helper extracted to `apps/backend/src/utils/redis_helpers.py` as part of this commit.
* `load_dotenv()` added to `main.py` for developer convenience.
...
* **[RESOLVED 2026-04-26 commit `21dbf77`] Authenticate `/api/v1/telemetry/history`.** Previously unauthenticated — anyone with network access could dump the full forensic GPS ledger via `?global=true`. Flagged in security sweep. **Resolution:** migrated from Flask to FastAPI in `telemetry_socket.py` with strict `OPS_HUB_TOKEN` verification dependency. The `?global=true` parameter now requires authenticated ops-hub access. 80 insertions, 25 deletions.

### Medium priority

* **[RESOLVED 2026-04-26 commit `ce6cdf8`] Worker mock endpoints.** `/api/v1/node/register`, `/api/v1/task/request`, `/api/v1/task/submit` were deleted along with `app.py` in Stage 1d-5. Decision: these were genuinely dead mock endpoints with no callers and weak shared-secret HMAC auth. Deleted rather than migrated.

* **[RESOLVED 2026-04-26 commit `ce6cdf8`] Decide the fate of the Flask `/api/v1/node/register` workflow.** Deleted. Nothing was calling it.

* **[RESOLVED 2026-04-26 commit `ce6cdf8`] Consolidate `decode_redis_hash` helper.** Extracted to `apps/backend/src/utils/redis_helpers.py` during the Stage 1d-5 Flask retirement refactor. All API modules now import from the shared utility.
...
* **[RESOLVED 2026-04-26 commit `ce6cdf8`] Hardware attestation substring check.** The `HW_SECURED = "0x8F9B" in MY_NODE_ID` magic-string check in `app.py` line 50 was deleted along with app.py. The post-pivot attestation path uses TPM-signed per-agent JWTs via `onboarding_api.py`, making the old substring check moot.
...
* **[RESOLVED 2026-04-26 commit `ce6cdf8`] Remove now-orphaned `load_optional_router` helper.** Deleted as part of Stage 1d-5.
...
* **[RESOLVED across commits `17bad8d`, `d0abeb0`, `2a44123`] Reconcile fee math across code, backend, and docs.** Three sources of truth previously disagreed. All three are now aligned.
    ...
    **Resolution history:**
    * **Backend** (`17bad8d`): `complete_mission` returns `net_payout` directly. `process_core_distress` locks platform fee tier at mission genesis.
    * **UI** (`d0abeb0`): `AgentDashboardScreen.kt` updated to read authoritative `netPayout` from backend response. Hardcoded `isVeteran = true` mock removed.
    * **Docs** (`2a44123`): `docs/API_SPEC.md` and `docs/legal/COMPLIANCE.md` updated to reflect the tiered agent fee structure (15%/25%) and platform fee structure (5%/10% with escrow threshold).
    
    **Remaining non-blocking follow-up:** Add contract/compliance note to `docs/legal/COMPLIANCE.md` clarifying that the fee structure is disclosed to agents in their onboarding agreement and to fleet partners in their API integration agreement. Legal should sign off on this language before Mesa launch, but this is a documentation follow-up, not a code blocker.
...
* **[RESOLVED 2026-04-26 commit `2a44123`] Draft `AZ_M2H_MANDATE.md` (Arizona M2H Physical Intervention Mandate).** Discovered during Stage 3-a audit. **Resolution:** `docs/legal/AZ_M2H_MANDATE.md` drafted (68 lines, version 2026.1.0 Mesa Pilot). Covers AZ Rev Stat § 28-9701 (SB 1417), agent authorization via TPM_PUB_KEY, scope of authority (ORP, perimeter clear, cabin/closure secure, scene secure), strict prohibitions (slashing offenses), liability shield with millisecond-precision UWB proximity lock transfer, and three automatic termination conditions. **Remaining non-blocking follow-up:** retained mobility counsel should review the drafted language before fleet-partner onboarding. The document uses placeholder fields (`[FLEET_ID]`, `[VEHICLE_VIN_HASH]`, etc.) that are filled programmatically at dispatch time.
...
* **[RESOLVED 2026-04-26 commit `d0abeb0`] Build real Firebase Auth end-to-end flow.** Previously: `PanWalletClient.kt` identity resolution fell back to `if (BuildConfig.IS_DEBUG) "DEV_AGENT_01" else null` with no real login screen. **Resolution:** real Firebase Auth wired into `LoginScreen.kt` (email/password authentication). `PanWalletClient.kt` updated to resolve agent identity from `FirebaseAuth.getInstance().currentUser`. The `DEV_AGENT_01` debug bypass has been destroyed. 4 files changed, 43 insertions, 27 deletions.

* **[RESOLVED across commits `17bad8d` and `d0abeb0`] Remove hardcoded `isVeteran` mock in `AgentDashboardScreen.kt`.** Previously: `val isVeteran = true  // MOCKED FEE STATUS for UI testing` caused every agent to get the veteran fee tier. **Resolution:** Backend side completed in `17bad8d` (`complete_mission` returns authoritative `net_payout`/`netPayout`). UI side completed in `d0abeb0` (`AgentDashboardScreen.kt` updated to read `netPayout` from the mission completion response, local `isVeteran` mock removed).
